...
void iExist()
{
    ...
    case SARType.FUNC_SAR:
        ...
        tempSymbol = _currentMethod.addTemporary(methodSymbol.type);
...
void newarr_sa()
{
    ...
    auto totalsz_symbol = _currentMethod.addTemporary("int");
    auto arr_symbol     = _currentMethod.addTemporary("@:"~type_sar.name);
...
void newobj_sa()
{
    ...
    auto mem_symbol = _currentMethod.addTemporary(type_sar.name);
    icode.malloc(class_symbol.offset, mem_symbol.id);

    // Call constructor
    auto temp_symbol = _currentMethod.addTemporary(type_sar.name);
...
void return_sa(size_t line)
{
    ...
    if (_sas.empty) {
        if (expectedRtnType != "void")
            throw new SemanticError(line,"Method ",_currentMethod.name," must return ",expectedRtnType,", not void");
        ...
        if (ret_symbol.type != expectedRtnType)
            throw new SemanticError(line,"Return statement for method ",_currentMethod.name," must be of type ",expectedRtnType,", not ",ret_symbol.type);
...
void doStackOp()
{
    ...
    case "/":
        ...
        auto temp_symbol = _currentMethod.addTemporary(l_symbol.type);
    ...
    case "!=":
        ...
        auto temp_symbol = _currentMethod.addTemporary("bool");
        _sas.push(SAR(SARType.TEMP_SAR,temp_symbol.name,l_sar.line,temp_symbol.id));
